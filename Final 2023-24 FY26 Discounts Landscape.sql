...
-- Data from Dec 2023 - Nov 2024
-- This query has two parts: first, it creates  temporary views for each month of tax year 2023 containing data such as auth_id, tax_year, pricing_segment, productFamily, and event_datetime; second, it creates a final table by unioning all these monthly views to capture only the earliest and latest event records for each auth ID.
...
-- DBTITLE 1,Commerce Pricing Segment Pipeline - Combining TY23-TY24
-- Query to combine commerce pricing data for TY23 and TY24
...
-- DBTITLE 1,Target State: discount_segments_base_23_24
...
a.tax_year,
      ...
      -- CASE WHEN a.metric_type = 'complete' then COALESCE( b.promotion_objective, c.promotion_objective, s.promotion_objective) 
      --      WHEN a.metric_type = 'start' THEN COALESCE( b.promotion_objective, c.promotion_objective) END AS  promotion_objective,
      ...
      -- CASE WHEN a.metric_type = 'complete' then COALESCE(cp.promotion_type , b.promotion_type, c.promotion_type,s.promotion_type, CASE WHEN d.list_price != m.total_federal_revenue THEN 'UNKNOWN_DISCOUNT' END) 
      --      WHEN a.metric_type = 'start' then COALESCE( b.promotion_type, c.promotion_type) END AS promotion_type,
